...
class RMT(nn.Module):
    ...
    def forward_and_adapt(self, x):
        ...
        outputs_test, features_test = self.model(x,return_feature=True)
        outputs_aug_test, features_aug_test = self.model(self.tta_transform((x)),return_feature=True)
        outputs_ema, features_ema     = self.model_ema(x,return_feature=True)
...
def collect_params(model):
    ...
    for nm, m in model.named_modules():
        if isinstance(m, (nn.BatchNorm2d, nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm)):
