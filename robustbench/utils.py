...
def load_model_bayes(model_name: str,
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        ...
        if dataset_ == BenchmarkDataset.imagenet or dataset_ == BenchmarkDataset.imagenet3d:
...
def clean_accuracy(model: nn.Module,
    ...
    with torch.no_grad():
        ...
        for counter in iterator:
            ...
            each_preds = np.append(each_preds, output.max(1)[1].cpu().numpy())

    return acc.item() / x.shape[0], each_preds
